...
class SpanMarkerOnnx:
    ...
    def data_to_device(self, data:torch.Tensor,classifier=False) -> None:
        if self.device == "cuda":
            if classifier:
                return data.to(self.device).numpy()
            return data.to(self.device).numpy().astype(np.int32)
        else:
            if classifier: 
...
def export_spanmarker_to_onnx(
    ...
    encoder_dummy_input = SpanMarkerEncoderDummyInputenerator.generate_dummy_input(vocab_size=config.vocab_size,sequence_length=config.model_max_length_default)
